...
async def owner_filter(_, client: BotClient, message: Message):
    ...
    return message.from_user.id == client.config["required"]["owner_id"]


async def authorized_only_filter(_, client: BotClient, message: Message):
    users_config = client.config.get("users", {})
    authorized_users = set(map(int, users_config.get("authorized_users", "").split()))
    authorized_chats = set(map(int, users_config.get("authorized_chats", "").split()))

    return (
        message.from_user.id == client.config["required"]["owner_id"]
